...
void I2C::open(uint8_t busId, uint8_t deviceAddress)
{
    ...
    auto result = i2cOpen(busId, deviceAddress, I2C_FLAGS);
    if (result == 0)
    {
        m_i2cHandle = result;
        m_busId = busId;
...
{
    auto result = i2cWriteByteData(m_i2cHandle, registerAddress, value);
    if (result == 0)
    {
        return;
    }
...
{
    auto result = i2cReadByteData(m_i2cHandle, registerAddress);
    if (result >= 0)
    {
...
{
    auto result = i2cReadWordData(m_i2cHandle, registerAddress);
    if (result >= 0)
    {
...
{
    auto result = i2cReadI2CBlockData(m_i2cHandle, registerAddress, (char*)data.data(), data.size());
    if (result >= 0)
    {
        return;
    }
